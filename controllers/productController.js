...
const updateProduct = async (req, res) => {
  try {
    ...
    // Validate that updates contain at least one of the expected fields
    const allowedUpdates = ['itemName', 'description', "price", 'colors', 'sizes', 'imagesToDelete'];
    const isValidUpdate = Object.keys(updates).every(update => allowedUpdates.includes(update));
...
const generateShareUrls = (product) => {
    ...
    const encodedProductUrl = encodeURIComponent(`http://yourwebsite.com/products/${product._id}`);
...
const updateSize = async (req, res) => {
    try {
      ...
      const sizeIndex = product.sizes.findIndex(sizeItem => sizeItem._id.toString() === sizeId);
      if (sizeIndex === -1) {
  ...
  const deleteSize = async (req, res) => {
    try {
      ...
      product.sizes = product.sizes.filter(size => size._id.toString() !== sizeId);
...
module.exports = { createProduct, updateProduct, getAllProducts, shareProduct, updateSize, compareProducts, deleteSize, updateColor, deleteColor, getProductById, deleteProduct }